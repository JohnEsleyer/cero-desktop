...
<script>
  ...
  // Full-Screen Reading View Bottom Panel States
  let showReadingBottomPanel = $state(false);
  let readingBottomPanelType = $state("scratchpad"); // "scratchpad" | "tallies"
  let readingBottomPanelHeight = $state(240);
  ...
  function onDragStart(panel, e) {
    ...
    } else if (panel === "reading_bottom_panel") {
      dragStartHeight = readingBottomPanelHeight;
    } else {
  ...
  function onDragMove(e) {
    ...
    } else if (dragging === "reading_bottom_panel") {
      const deltaY = e.clientY - dragStartY;
      readingBottomPanelHeight = Math.max(120, Math.min(600, dragStartHeight - deltaY));
    }
  }
...
<!-- Fullscreen Reading Modal with Dedicated Context Notes Panel -->
{#if showReadingFullscreenModal && readingCard}
  <div class="fullscreen-editor-overlay" style="display: flex; flex-direction: column; background: #09090b; height: 100vh; width: 100vw;">
    <div class="fullscreen-header">
      ...
      <div class="header-buttons" style="display: flex; gap: 8px;">
        <button
          ...
          onclick={() => {
            if (showReadingBottomPanel && readingBottomPanelType === "scratchpad") {
              showReadingBottomPanel = false;
            } else {
              showReadingBottomPanel = true;
              readingBottomPanelType = "scratchpad";
            }
          }}
        >
          📝 Scratchpad
        ...
        <button
          class="btn-sm"
          style="background: {showReadingBottomPanel && readingBottomPanelType === 'tallies' ? 'rgba(129, 140, 248, 0.15)' : 'rgba(255, 255, 255, 0.02)'}; border-color: {showReadingBottomPanel && readingBottomPanelType === 'tallies' ? '#818cf8' : 'rgba(255, 255, 255, 0.05)'}; color: {showReadingBottomPanel && readingBottomPanelType === 'tallies' ? '#818cf8' : '#cbd5e1'}; font-weight: bold;"
          onclick={() => {
            if (showReadingBottomPanel && readingBottomPanelType === "tallies") {
              showReadingBottomPanel = false;
            } else {
              showReadingBottomPanel = true;
              readingBottomPanelType = "tallies";
              FetchCards("global-tally");
            }
          }}
        >
        ...
        <button
          class="btn-sm"
          style="background: {showReadingContextSidebar ? 'rgba(129, 140, 248, 0.15)' : 'rgba(255, 255, 255, 0.02)'}; border-color: {showReadingContextSidebar ? '#818cf8' : 'rgba(255, 255, 255, 0.05)'}; color: {showReadingContextSidebar ? '#818cf8' : '#cbd5e1'}; font-weight: bold;"
          onclick={() => (showReadingContextSidebar = !showReadingContextSidebar)}
        >
          📋 Context Notes
        </button>
    ...
    <div class="fullscreen-workspace" style="display: flex; flex-direction: row; flex: 1; overflow: hidden; position: relative; width: 100%;">
      ...
      <div style="flex: 1; display: flex; flex-direction: column; overflow: hidden; position: relative;">
        ...
        <!-- Resizable Dedicated Bottom Dock for Scratchpad / Tallies -->
        {#if showReadingBottomPanel}
          <!-- svelte-ignore a11y-no-static-element-interactions -->
          <div
            onmousedown={(e) => onDragStart("reading_bottom_panel", e)}
            class:active={dragging === "reading_bottom_panel"}
            style="height: 4px; cursor: row-resize; display: flex; align-items: center; justify-content: center; flex-shrink: 0; background: transparent; z-index: 200; width: 100%;"
          >
            <div style="width: 32px; height: 1px; background: rgba(255, 255, 255, 0.15);"></div>
          </div>

          <div
            style="height: {readingBottomPanelHeight}px; min-height: 120px; max-height: 600px; display: flex; flex-direction: column; background: #0c0c0e; border-top: 1px solid rgba(255, 255, 255, 0.05); overflow: hidden; flex-shrink: 0; width: 100%;"
          >
            {#if readingBottomPanelType === "scratchpad"}
              <div class="scratchpad-header" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; border-bottom: 1px solid rgba(255,255,255,0.05); background: #121215; flex-shrink: 0;">
                <span style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #818cf8;">Reading Scratchpad</span>
                <button class="close-btn" style="font-size: 14px; padding: 2px 6px;" onclick={() => (showReadingBottomPanel = false)}>&times;</button>
              </div>
              <textarea
                id="reading-scratchpad-textarea"
                style="flex: 1; background: transparent; border: none; resize: none; outline: none; color: #e4e4e7; font-size: 12px; line-height: 1.6; padding: 14px;"
                bind:value={scratchpadContent}
                oninput={saveScratchpadDebounced}
                placeholder="Take notes here while reading..."
              ></textarea>
            {:else if readingBottomPanelType === "tallies"}
              <div class="scratchpad-header" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; border-bottom: 1px solid rgba(255,255,255,0.05); background: #121215; flex-shrink: 0;">
                <span style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #818cf8;">Reading Tallies</span>
                <button class="close-btn" style="font-size: 14px; padding: 2px 6px;" onclick={() => (showReadingBottomPanel = false)}>&times;</button>
              </div>
              <div style="flex: 1; overflow-y: auto; padding: 10px; display: flex; flex-direction: column; gap: 8px;">
                {#each tallyCards as card (card.id)}
                  {@const data = (() => { try { return JSON.parse(card.content); } catch (_) { return { name: card.comment || "Tally", count: parseInt(card.content) || 0 }; } })()}
                  <div style="display: flex; align-items: center; justify-content: space-between; background: #18181b; border: 1px solid rgba(255, 255, 255, 0.04); border-radius: 8px; padding: 8px 12px;">
                    <span style="font-size: 12.5px; font-weight: 700; color: #e4e4e7;">{data.name}</span>
                    <div style="display: flex; align-items: center; gap: 4px;">
      ...
      <!-- Context Notes Right Sidebar for Reading Card -->
      {#if showReadingContextSidebar}
        <aside class="sidebar right-sidebar" style="width: 280px; min-width: 280px; border-left: 1px solid rgba(255, 255, 255, 0.05);">
          <RightSidebar
...
<!-- Immersive Sites Sandbox Preview Modal -->
{#if showSitesPreviewModal}
  ...
  >
    <div
      class="modal-container"
      style="width: 1000px; max-width: 95%; height: 680px; max-height: 90vh; display: flex; flex-direction: column;"
    >
      <div class="modal-header">
        <div style="display: flex; flex-direction: column; gap: 2px;">
          <h3 style="margin:0;">🎬 Live Sandbox Preview - {previewSitesName}</h3>
          <span style="font-size: 11px; color: #64748b;">Interacting in a secure sandbox webview container</span>
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
          <button
            class="btn-sm"
            style="background: {showSitesContextSidebar ? 'rgba(129, 140, 248, 0.15)' : 'rgba(255, 255, 255, 0.02)'}; border-color: {showSitesContextSidebar ? '#818cf8' : 'rgba(255, 255, 255, 0.05)'}; color: {showSitesContextSidebar ? '#818cf8' : '#cbd5e1'}; font-weight: bold;"
            onclick={() => (showSitesContextSidebar = !showSitesContextSidebar)}
          >
            📋 Context Notes
          </button>
      ...
      <div class="modal-body" style="flex:1; display: flex; overflow:hidden; padding: 0; background: #09090b;">
        ...
        {#if showSitesContextSidebar}
          <aside class="sidebar right-sidebar" style="width: 280px; min-width: 280px; border-left: 1px solid rgba(255, 255, 255, 0.05); height: 100%;">
            <RightSidebar
              activeCard={activeCardForContext}
              onAddContextNote={handleOpenAddContextNote}
              onOpenContextNote={openContextNoteFullscreen}
